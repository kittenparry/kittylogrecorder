...
class Gui(tk.Frame):
	...
	def add_elements(self):
		...
		self.label_file_name = tk.Label(self.row_top, text=strings("label_file_name"), state='disabled')
		self.entry_file_name = tk.Entry(self.row_top, width=10)
		self.entry_file_name.insert(tk.END, strings("name_file_name"))
	...
	def log_entry(self):
		try:
			...
			file = str(self.entry_file_name.get())

			# if filename doesn't end in .txt append it
			# if path ends in backward or forward slash append file name to path
